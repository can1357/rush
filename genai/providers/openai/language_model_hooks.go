...
type LanguageModelPrepareCallFunc = func(model genai.LanguageModel, params *openai.ChatCompletionNewParams, call genai.Call) ([]genai.CallWarning, error)

// LanguageModelMapFinishReasonFunc is a function that maps the finish reason for the language model.
type LanguageModelMapFinishReasonFunc = func(finishReason string) genai.FinishReason

// LanguageModelUsageFunc is a function that calculates usage for the language model.
type LanguageModelUsageFunc = func(choice openai.ChatCompletion) (genai.Usage, genai.ProviderOptionsData)

// LanguageModelExtraContentFunc is a function that adds extra content for the language model.
type LanguageModelExtraContentFunc = func(choice openai.ChatCompletionChoice) []genai.Content

// LanguageModelStreamExtraFunc is a function that handles stream extra functionality for the language model.
type LanguageModelStreamExtraFunc = func(chunk openai.ChatCompletionChunk, yield func(genai.StreamPart) bool, ctx map[string]any) (map[string]any, bool)

// LanguageModelStreamUsageFunc is a function that calculates stream usage for the language model.
type LanguageModelStreamUsageFunc = func(chunk openai.ChatCompletionChunk, ctx map[string]any, metadata genai.ProviderMetadata) (genai.Usage, genai.ProviderMetadata)

// LanguageModelStreamProviderMetadataFunc is a function that handles stream provider metadata for the language model.
type LanguageModelStreamProviderMetadataFunc = func(choice openai.ChatCompletionChoice, metadata genai.ProviderMetadata) genai.ProviderMetadata

// LanguageModelToPromptFunc is a function that handles converting fantasy prompts to openai sdk messages.
type LanguageModelToPromptFunc = func(prompt genai.Prompt, provider, model string) ([]openai.ChatCompletionMessageParamUnion, []genai.CallWarning)

// DefaultPrepareCallFunc is the default implementation for preparing a call to the language model.
func DefaultPrepareCallFunc(model genai.LanguageModel, params *openai.ChatCompletionNewParams, call genai.Call) ([]genai.CallWarning, error) {
	...
	if v, ok := call.ProviderOptions[Name]; ok {
		...
		if !ok {
			return nil, &genai.Error{Title: "invalid argument", Message: "openai provider options should be *openai.ProviderOptions"}
		}
	}
...
func DefaultMapFinishReasonFunc(finishReason string) genai.FinishReason {
	switch finishReason {
	case "stop":
...
func DefaultUsageFunc(response openai.ChatCompletion) (genai.Usage, genai.ProviderOptionsData) {
...
func DefaultStreamUsageFunc(chunk openai.ChatCompletionChunk, _ map[string]any, metadata genai.ProviderMetadata) (genai.Usage, genai.ProviderMetadata) {
	...
	return usage, genai.ProviderMetadata{
...
func DefaultStreamProviderMetadataFunc(choice openai.ChatCompletionChoice, metadata genai.ProviderMetadata) genai.ProviderMetadata {
	if metadata == nil {
...
func DefaultToPrompt(prompt genai.Prompt, _, _ string) ([]openai.ChatCompletionMessageParamUnion, []genai.CallWarning) {
