...
func New(opts ...Option) (genai.Provider, error) {
	providerOptions := options{
		...
		objectMode: genai.ObjectModeTool, // Default to tool mode for openai-compat
	}
	for _, o := range opts {
		o(&providerOptions)
	}
	...
	if objectMode == genai.ObjectModeAuto || objectMode == genai.ObjectModeJSON {
		objectMode = genai.ObjectModeTool
	}
