		...
		prompt := genai.Prompt{
			{
				...
				Content: []genai.MessagePart{
					genai.TextPart{Text: "What is 2+2?"},
				},
			},
			{
				Role: genai.MessageRoleAssistant,
				Content: []genai.MessagePart{
					genai.ReasoningPart{Text: "Let me think... 2+2 equals 4."},
					genai.TextPart{Text: "The answer is 4."},
				},
			},
			{
				Role: genai.MessageRoleUser,
				Content: []genai.MessagePart{
					genai.TextPart{Text: "What about 3+3?"},
				},
			},
		}
	...
	t.Run("should handle assistant messages with only reasoning content", func(t *testing.T) {
		...
		prompt := genai.Prompt{
			...
			},
			{
				Role: genai.MessageRoleAssistant,
				Content: []genai.MessagePart{
					genai.ReasoningPart{Text: "Internal reasoning only..."},
				},
			},
		}
		...
		prompt := genai.Prompt{
			{
				...
				Content: []genai.MessagePart{
					genai.TextPart{Text: "You are helpful."},
				},
			},
			{
				Role: genai.MessageRoleUser,
	...
	t.Run("should use last assistant TextPart only", func(t *testing.T) {
		...
		prompt := genai.Prompt{
			...
			},
			{
				Role: genai.MessageRoleAssistant,
				Content: []genai.MessagePart{
					genai.TextPart{Text: "First part. "},
					genai.TextPart{Text: "Second part. "},
					genai.TextPart{Text: "Third part."},
				},
			},
		}
	...
	t.Run("should include user messages with only unsupported attachments", func(t *testing.T) {
		...
		prompt := genai.Prompt{
			...
			},
			{
				Role: genai.MessageRoleUser,
				Content: []genai.MessagePart{
					genai.TextPart{Text: "After unsupported"},
				},
			},
		}
		...
		prompt := genai.Prompt{
			{
				Role: genai.MessageRoleUser,
				Content: []genai.MessagePart{
					genai.TextPart{Text: "Check this PDF"},
					genai.FilePart{
