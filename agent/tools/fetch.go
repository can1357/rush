...
func NewFetchTool(permissions permission.Service, workingDir string, client *http.Client) genai.AgentTool {
	if client == nil {
		client = &http.Client{
	...
	return genai.NewAgentTool(
		...
		func(ctx context.Context, params FetchParams, call genai.ToolCall) (genai.ToolResponse, error) {
			if params.URL == "" {
			...
			if format != "text" && format != "markdown" && format != "html" {
				return genai.NewTextErrorResponse("Format must be one of: text, markdown, html"), nil
			}
			...
			if err != nil {
				return genai.NewTextErrorResponse("Failed to read response body: " + err.Error()), nil
			}
			...
			if !isValidUt8 {
				return genai.NewTextErrorResponse("Response content is not valid UTF-8"), nil
			}
			...
			case "text":
				if strings.Contains(contentType, "text/html") {
					...
					if err != nil {
						return genai.NewTextErrorResponse("Failed to extract text from HTML: " + err.Error()), nil
					}
					content = text
				}
			...
			case "markdown":
				if strings.Contains(contentType, "text/html") {
					...
					if err != nil {
						return genai.NewTextErrorResponse("Failed to convert HTML to Markdown: " + err.Error()), nil
					}
					content = markdown
				}
			...
			case "html":
				...
				if strings.Contains(contentType, "text/html") {
					...
					if err != nil {
						return genai.NewTextErrorResponse("Failed to parse HTML: " + err.Error()), nil
					}
					body, err := doc.Find("body").Html()
					if err != nil {
						return genai.NewTextErrorResponse("Failed to extract body from HTML: " + err.Error()), nil
					}
					if body == "" {
						return genai.NewTextErrorResponse("No body content found in HTML"), nil
					}
					content = "<html>\n<body>\n" + body + "\n</body>\n</html>"
				}
			}
