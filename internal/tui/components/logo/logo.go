// Package logo renders a Rush wordmark in a stylized way.
package logo
...
// Opts are the options for rendering the Rush title art.
type Opts struct {
...
// Render renders the Rush logo. Set the argument to true to render the narrow
...
func Render(version string, compact bool, o Opts) string {
	...
	rush := renderWord(spacing, stretchIndex, letterforms...)
	rushWidth := lipgloss.Width(rush)
	b := new(strings.Builder)
	for r := range strings.SplitSeq(rush, "\n") {
	...
	// Charm and version.
	metaRowGap := 1
	maxVersionWidth := rushWidth - lipgloss.Width(charm) - metaRowGap
	version = ansi.Truncate(version, maxVersionWidth, "…") // truncate version if too long.
	gap := max(0, rushWidth-lipgloss.Width(charm)-lipgloss.Width(version))
	metaRow := fg(o.CharmColor, charm) + strings.Repeat(" ", gap) + fg(o.VersionColor, version)

	// Join the meta row and big Rush title.
	rush = strings.TrimSpace(metaRow + "\n" + rush)

	// Narrow version.
	if compact {
		field := fg(o.FieldColor, strings.Repeat(diag, rushWidth))
		return strings.Join([]string{field, field, rush, field, ""}, "\n")
	}

	fieldHeight := lipgloss.Height(rush)
	...
	// Right field.
	rightWidth := max(15, o.Width-rushWidth-leftWidth-2) // 2 for the gap.
	const stepDownAt = 0
	...
	const hGap = " "
	logo := lipgloss.JoinHorizontal(lipgloss.Top, leftField.String(), hGap, rush, hGap, rightField.String())
	if o.Width > 0 {
...
// SmallRender renders a smaller version of the Rush logo, suitable for
// smaller windows or sidebar usage.
func SmallRender(width int) string {
	t := styles.CurrentTheme()
	title := t.S().Base.Foreground(t.Secondary).Render("Charm™")
	title = fmt.Sprintf("%s %s", title, styles.ApplyBoldForegroundGrad("Rush", t.Secondary, t.Primary))
	remainingWidth := width - lipgloss.Width(title) - 1 // 1 for the space after "Rush"
	if remainingWidth > 0 {
