...
func TestRushIgnore(t *testing.T) {
	tempDir := t.TempDir()
	t.Chdir(tempDir)
	...
	// Create a .rushignore file that ignores .log files
	require.NoError(t, os.WriteFile(".rushignore", []byte("*.log\n"), 0o644))
...
func TestShouldExcludeFile(t *testing.T) {
	...
	require.True(t, ShouldExcludeFile(tempDir, customIgnored), "Expected custom_ignored to be ignored by .rushignore")
...
func TestShouldExcludeFileHierarchical(t *testing.T) {
	...
	// Create .rushignore in subdir that ignores normal_nested
	subRushIgnore := "normal_nested/\n"
	if err := os.WriteFile(filepath.Join(subDir, ".rushignore"), []byte(subRushIgnore), 0o644); err != nil {
		t.Fatalf("Failed to create subdir .rushignore: %v", err)
	}

	// Test hierarchical ignore behavior - this should work because the .rushignore is in the parent directory
	require.True(t, ShouldExcludeFile(tempDir, nestedNormal), "Expected normal_nested to be ignored by subdir .rushignore")
